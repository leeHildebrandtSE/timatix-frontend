...
const ServiceRequests = ({ navigation }) => {
  ...
  const loadServiceRequests = async () => {
    if (!user?.id) {
      console.error('No user ID found. Cannot fetch service requests.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to load service requests. Using fallback data.');
      ...
      const mockRequests = [
        ...
        },
        {
          ...
          assignedMechanic: { id: 'm1', name: 'Mike Smith' },
        ...
        },
        {
          ...
          vehicle: { id: '1', make: 'Toyota', model: 'Corolla', year: 2020 },
          quote: { id: 'q2', totalAmount: 450.00 },
          assignedMechanic: { id: 'm1', name: 'Mike Smith' },
  ...
  const handleCreateServiceRequest = () => navigation.navigate('CreateServiceRequest');
  const handleServicePress = (service) => navigation.navigate('ServiceDetails', { serviceId: service.id });
  const handleViewQuote = (service) => navigation.navigate('QuoteDetails', { serviceId: service.id, quoteId: service.quote?.id });
          ...
          onPress: async () => {
            try {
              ...
              const updatedRequests = serviceRequests.map(req =>
                req.id === service.id ? { ...req, status: SERVICE_STATUS.APPROVED } : req
              );
              setServiceRequests(updatedRequests);
              addNotification({ title: 'Quote Accepted', message: `Quote for ${service.serviceType} has been accepted.`, type: 'success' });
            } catch (error) {
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const updatedRequests = serviceRequests.map(req =>
                req.id === service.id ? { ...req, status: SERVICE_STATUS.DECLINED } : req
              );
              setServiceRequests(updatedRequests);
              addNotification({ title: 'Quote Declined', message: `Quote for ${service.serviceType} has been declined.`, type: 'info' });
            } catch (error) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, theme.typography.h3]}>Service Requests</Text>
        <Button title="+ New" onPress={handleCreateServiceRequest} style={styles.addButton} />
      </View>
      ...
      <View style={styles.filters}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.statusFilters}>
          {getStatusOptions().map(status => (
            <TouchableOpacity
              key={status}
              style={[styles.statusFilter, selectedStatus === status && { backgroundColor: theme.colors.primary }]}
              onPress={() => setSelectedStatus(status)}
            >
              <Text style={[styles.statusFilterText, selectedStatus === status ? { color: '#fff' } : { color: theme.colors.text }]}>
        ...
        {filteredRequests.length > 0 ? (
          ...
          <View style={[styles.emptyState, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.emptyStateTitle, theme.typography.h6]}>
              {searchQuery || selectedStatus !== 'ALL' ? 'No requests found' : 'No service requests yet'}
            </Text>
            <Text style={[styles.emptyStateText, theme.typography.body2]}>
              {searchQuery || selectedStatus !== 'ALL' ? 'Try adjusting your search or filters' : 'Create your first service request to get started'}
            </Text>
            {!searchQuery && selectedStatus === 'ALL' && (
              <Button title="Create Service Request" onPress={handleCreateServiceRequest} style={styles.emptyStateButton} />
            )}
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  searchInput: { height: 44, borderRadius: 8, paddingHorizontal: 16, borderWidth: 1 },
  statusFilters: { flexDirection: 'row' },
  statusFilter: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#F5F5F5', marginRight: 8 },
  statusFilterText: { fontSize: 14, fontWeight: '500' },
  requestCount: { paddingHorizontal: 20, paddingBottom: 8 },
  requestCountText: { opacity: 0.7 },
  scrollView: { flex: 1 },
  scrollContent: { paddingHorizontal: 20, paddingBottom: 20 },
  emptyState: { padding: 48, borderRadius: 12, alignItems: 'center', marginTop: 60, borderWidth: 1, borderColor: '#E0E0E0', borderStyle: 'dashed' },
  emptyStateTitle: { marginBottom: 8, textAlign: 'center' },
  emptyStateText: { marginBottom: 24, textAlign: 'center', opacity: 0.6 },
